...
export const GetEmbededText = (event: TypeEvent): string => {
    ...
    return [
        ...
        line(`**Issued:**`, `<t:${Math.floor(new Date(event.properties.issued).getTime() / 1000)}:R>`, isIssued),
        line(`**Updated:**`, `<t:${Math.floor(new Date(event.properties.issued).getTime() / 1000)}:R>`, isUpdated && !isExpired),
        line(`**Expires:**`, `<t:${Math.floor(new Date(event.properties.expires).getTime() / 1000)}:R>`, !isStatement && !isExpired),
        line(`**Expired:**`, `<t:${Math.floor(new Date(event.properties.expires).getTime() / 1000)}:R>`, isExpired),
