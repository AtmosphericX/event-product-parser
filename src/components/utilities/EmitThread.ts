...
export const EmitThread = async (): Promise<void> => {
    ...
    const walk = (dir: string, deleteFolder = false): void => {
        ...
        if (deleteFolder) {
            try { 
                if (readdirSync(dir).length === 0 && statSync(dir).mtime.getTime() < (Date.now() - 3600000)) { rmdirSync(dir); } 
                if (readdirSync(dir).length === 0 && statSync(dir).mtime.getTime() < TTLCUT) { rmdirSync(dir); }
            } catch (err) {}
        }
    }
    ...
    if (settings.EnableWireService) {
        ...
        await ProcessStanza({ Message: response.message, NWWS: false })
    }
}
