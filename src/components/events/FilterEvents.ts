...
import { GetEventEnhancedName } from "@BuilderComponents/GetEventEnhancedName"
import { GetEventSignature } from "@BuilderComponents/GetEventSignature"
import { CreateEvents } from "@Builder/CreateEvents"
import { RemoveEvents } from "@Builder/RemoveEvents"
import { GetEventAttachments } from "@BuilderComponents/GetEventAttachments"
import { SetEventEmit } from "@Utilities/SetEventEmit"
import { SetDebug } from "@Utilities/SetDebug"
import { GetMatched } from "@Utilities/GetMatched"
import { GetEventPopulation } from "@BuilderComponents/GetEventPopulation"
import { GetEventGeometry } from "@BuilderComponents/GetEventGeometry"
import { createHash } from "crypto"

export const FilterEvents = async (events: TypeEvent[]): Promise<void> => {
    ...
    const isFiltered = (define: TypeEvent): boolean => {
        ...
        if (properties.status_metadata.is_expired) { 
            ...
            RemoveEvents({ Event: define, IsTimeBasedExpiration: false })
            return true; 
        }
        ...
        if (properties.metadata?.vtec?.watch && properties.metadata.source != `events.api`) {
            ...
            SetEventEmit({ Event: isWOU ? `onWatchOutlineUpdate` : `onWatchCountyNotification`, Metadata: define })
        }
